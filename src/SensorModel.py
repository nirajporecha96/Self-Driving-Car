...
SIGMA_HIT = 2.00 # Noise value for hit reading
Z_HIT     = 0.10 # Weight for hit reading
...
class SensorModel:
  ...
  def __init__(self, scan_topic, laser_ray_step, exclude_max_range_rays, 
    ...
    self.max_range_px = int(self.MAX_RANGE_METERS / self.map_info.resolution) # The max range in pixels of the laser
    self.range_method = range_libc.PyCDDTCast(oMap, self.max_range_px, THETA_DISCRETIZATION) # The range method that will be used for ray casting
    #self.range_method = range_libc.PyRayMarchingGPU(oMap, max_range_px) # The range method that will be used for ray casting
    self.range_method.set_sensor_model(self.precompute_sensor_model(self.max_range_px)) # Load the sensor model expressed as a table
  ...
  def lidar_cb(self, msg):
    ...
    self.downsampled_ranges = np.array(msg.ranges[::self.LASER_RAY_STEP]).astype(np.float32)
    self.downsampled_ranges[np.isnan(self.downsampled_ranges)] = self.MAX_RANGE_METERS 
    self.downsampled_ranges[self.downsampled_ranges[:] == 0] = self.MAX_RANGE_METERS
    
    for x in xrange(len(self.downsampled_ranges)):
        if(math.isnan(self.downsampled_ranges[x])):
            self.downsampled_ranges[x] = self.MAX_RANGE_METERS 

    if len(self.downsampled_ranges) == len(self.downsampled_angles):
        pass

    obs = (self.downsampled_ranges, self.downsampled_angles)
