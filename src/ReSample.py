...
class ReSampler:
  ...
  def resample_naiive(self):
    ...
    m_bins = np.random.choice(n_particles, n_particles, replace=True)
    m_particles = np.zeros((n_particles,3))
    m_weights = np.zeros(n_particles)
    
    print self.weights
    for i in xrange(n_particles):
        chosen_bin = m_bins[i]
        m_weights[i] = self.weights[chosen_bin]
        m_particles[i,:] = self.particles[chosen_bin,:]

    self.particles[:,:] = m_particles[:,:]
  ...
  def resample_low_variance(self):
    ...
    for m in xrange (0, n_particles):
      ...
      m_particles[m,:] = self.particles[i,:]
