...
class LineFollower:
  ...
  '''
  Initializes the line follower plan: A list of length T that represents the path that the robot should follow each element of the list is a 3-element numpy array of the form [x,y,theta]

  pose_topic: The topic that provides the current pose of the robot as a PoseStamped msg

  plan_lookahead: If the robot is currently closest to the i-th pose in the plan, then it should navigate towards the (i+plan_lookahead)-th pose in the plan

  translation_weight: How much the error in translation should be weighted in relation to the error in rotation

  rotation_weight: How much the error in rotation should be weighted in relation to the error in translation
  ...
  def __init__(self, plan, pose_topic, plan_lookahead, translation_weight, rotation_weight, kp, ki, kd, error_buff_length, speed):
    ...
    self.ki = ki
    self.kd = kd
    # The error buff stores the error_buff_length most recent errors and the times at which they were received. That is, each element is of the form[time_stamp (seconds), error]. For more info about the data struct itself, visit
  ...
  Returns: (False, 0.0) if the end of the plan has been reached. Otherwise, returns  (True, E) - where E is the computed error
  '''
  ...
  def compute_error(self, cur_pose):
    ...
    # YOUR CODE HERE
      ...
      trans_v = [[self.plan[0][0]-cur_pose[0]],[self.plan[0][1]-cur_pose[1]]]
    ...
    trans_ve = [[self.plan[goal_idx][0]-cur_pose[0]], [self.plan[goal_idx][1]-cur_pose[1]]]
  ...
  def compute_steering_angle(self, error):
    ...
    deriv_error=(error-self.error_buff[0][0]) / (now-self.error_buff[0][1])
    ...
    for i in range (0,len(self.error_buff)):
      integ_error += self.error_buff[i][0]

    # Compute the steering angle as the sum of the pid errors
    # YOUR CODE HERE
    str_ang = self.kp*error + self.ki*integ_error + self.kd*deriv_error[0]
    return str_ang
...
def main():
  ...
  plan = []
  a=rospy.wait_for_message(plan_topic,PoseArray)
  for i in a.poses:
    plan.append(np.array([i.position.x,i.position.y,utils.quaternion_to_angle(i.orientation)]))
