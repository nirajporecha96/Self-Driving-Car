...
KM_THETA_FIX_NOISE = 0.20 # Kinematic car theta constant noise std dev
...
class KinematicMotionModel:
  ...
  def __init__(self, motor_state_topic, servo_state_topic, speed_to_erpm_offset, speed_to_erpm_gain, steering_to_servo_offset, steering_to_servo_gain, car_length, particles, state_lock=None):
    ...
    # This subscriber just catches the most recent servo position command
  ...
  '''
    Catches the most recent servo command
  ...
  def motion_cb(self, msg):
    ...
    # YOUR CODE HERE
    curr_steer = (self.last_servo_cmd - self.STEERING_TO_SERVO_OFFSET) / self.STEERING_TO_SERVO_GAIN

    curr_speed_wnoise = np.random.normal(curr_speed, KM_V_NOISE, len(self.particles))
    curr_steer_wnoise = np.random.normal(curr_steer, KM_DELTA_NOISE, len(self.particles))
    ...
    # YOUR CODE HERE
    td = msg.header.stamp - self.last_vesc_stamp
    t = td.to_sec()

    beta = np.arctan(0.5*np.tan(curr_steer_wnoise))
    theta1 = self.particles[:,2]
    theta = theta1 + (curr_speed_wnoise / self.CAR_LENGTH) * np.sin(2*beta) * t
    theta = ((theta + np.pi) % (2*np.pi)) - np.pi
    self.particles[:,0] +=  ((self.CAR_LENGTH/np.sin(2*beta))* (np.sin(theta) - np.sin(theta1)))
    self.particles[:,1] +=  ((self.CAR_LENGTH/np.sin(2*beta))* -(np.cos(theta) - np.cos(theta1)))

    self.particles[:,0] = np.random.normal(self.particles[:,0], KM_X_FIX_NOISE, len(self.particles))
    self.particles[:,1] = np.random.normal(self.particles[:,1], KM_Y_FIX_NOISE, len(self.particles))
    self.particles[:,2] = np.random.normal(theta, KM_THETA_FIX_NOISE, len(self.particles))
...
if __name__ == '__main__':
  ...
  kmm = KinematicMotionModel(motor_state_topic, servo_state_topic, speed_to_erpm_offset, speed_to_erpm_gain, steering_angle_to_servo_offset, steering_angle_to_servo_gain, car_length,particles)
