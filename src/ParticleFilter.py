...
class ParticleFilter():
  ...
  def __init__(self, n_particles, n_viz_particles,
    ...
    # Subscribe to the '/initialpose' topic. Published by RVIZ. See clicked_pose_cb function in this file for more info
  ...
  def expected_pose(self):
    ...
    for i in range(len(self.particles)):
        tempx.append(self.weights[i] * self.particles[i][0])
        tempy.append(self.weights[i] * self.particles[i][1])
        tempsin.append(np.sin(self.particles[i][2]))
        tempcos.append(np.cos(self.particles[i][2]))
    expx = np.sum(tempx)
    expy = np.sum(tempy)
    exptheta = np.arctan2(np.sum(tempsin), np.sum(tempcos))
    return np.array([expx, expy, exptheta])
  ...
  def clicked_pose_cb(self, msg):
    ...
    # YOUR CODE HERE
    x_curr = np.full((self.N_PARTICLES), msg.pose.pose.position.x)
    y_curr = np.full((self.N_PARTICLES), msg.pose.pose.position.y)
    theta_curr = np.full((self.N_PARTICLES), Utils.quaternion_to_angle(msg.pose.pose.orientation))
    x_std = 0.03
    y_std = 0.03
    theta_std = 0.2
    x_noise = np.random.normal(0, x_std, self.N_PARTICLES)
    y_noise = np.random.normal(0, y_std, self.N_PARTICLES)
    theta_noise = np.random.normal(0 ,theta_std, self.N_PARTICLES)
    self.particles[:,0] = x_curr[:] + x_noise[:]
    self.particles[:,1] = y_curr[:] + y_noise[:]
    self.particles[:,2] = theta_curr[:] + theta_noise[:] 
    self.weights[:] = 1.0 / float(self.N_PARTICLES)
  ...
  def visualize(self):
    ...
    if self.particle_pub.get_num_connections() > 0:
      if self.particles.shape[0] > self.N_VIZ_PARTICLES:
        ...
        proposal_indices = np.random.choice(self.particle_indices, self.N_VIZ_PARTICLES, replace=False, p=self.weights)
